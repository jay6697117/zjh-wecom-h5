...
const configDefault = {
  headers: {
    'Content-Type': ContentTypeEnum.JSON as string
  },
  timeout: 15000,
 ...
 * @description: useHttpHook函数不传参数是application/json，传了参数按照枚举值
 */
export function useHttpHook(otherHeader?: string) {
  if (otherHeader) {
    configDefault.headers['Content-Type'] = otherHeader
  }
  return new Http(configDefault)
}
